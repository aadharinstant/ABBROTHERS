...
$res = mysqli_query($conn, "SELECT * FROM `customers` WHERE appliedby='$s_phone' AND status='success' AND type='2' OR appliedby='$s_phone' AND status='success' AND type='3' OR appliedby='$s_phone' AND status='success' AND type='4' OR  appliedby='$s_phone' AND status='success' AND type='6'");
if(isset($_POST['delete']) && $_POST['delete'] == "ahkwebsolutions"){
  ...
  window.location.href = "demoallsuccess.php";
  }, 1500);
        </script>
        <?php
    }else{
        ...
        <div class="col-lg-6 align-self-center text-right">
            <a href="add-customer.php" class="btn btn-success box-shadow btn-icon btn-rounded"><i class="fa fa-plus"></i> Create New</a>
                    ...
                    <div class="card-header card-default">
                        View Customers Data - Super Admin 3 </div>
                        <div class="card-body">
                        <!-- /.panel-heading -->
                           ...
                           <table id="table_id" class="table table-striped table-bordered table-hover">
                                ...
                                <thead>
                                    ...
                                    <th style="width: 50px;" class="text-center">S/No</th>
                                        <th style="width: 10px;" class="text-center">Applied By</th>
                                        ...
                                        <th style="width: 20px;" class="text-center">Purpose</th>
                                ...
                                <?php
                                    if(mysqli_num_rows($res)>0){
                                        ...
                                        while($data = mysqli_fetch_array($res)){
                                            ...
                                            <td class="text-center text-danger"> <h6 style="background:#36b9cc;margin:0 auto;width:max-content;padding:0 10px;color:#fff;border-radius:4px"><?php echo $data['parent'] ?></h6> 
                                                </td>
                                                <td>Old - <?php echo $data['name'] ?>
												  <?php if(!empty($data['new_name'])){ echo '<br />New Name - ' .$data['new_name']; } ?>
												</td>
                                                <td class="text-center"><?php echo $data['aadhaar_no'] ?></td>
                                              
                                                <td><?php echo $data['mobile_no'] ?></td>
                                                <td>Old - <?php echo $data['gender'] ?><?php if(!empty($data['gender'])){ echo '<br />New Gender - ' .$data['new_gender']; } ?></td>
                                                
                                                 <td class="text-center"> Old - <?php echo $data['dob'] ?><?php if(!empty($data['dob'])){ echo '<br />New DOB - ' .$data['new_dob']; } ?>
                                                </td>
												<td class="text-center"> <?php echo $data['address'] ?>
                                                <td class="text-center"> <?php echo $data['purpose'] ?>
                                                </td>
                                                <td class="text-center"> <a href="editapplication.php?id=<?php echo base64_encode($data['id']) ?>" button class="btn btn-success"><?php echo $data['status'] ?></button></a>
                                                </td>
